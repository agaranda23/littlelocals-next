...
export default function ListingCard({ listing }) {
  ...
  const card = (
    <div style={{ background: 'white', borderRadius: 16, overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.07)', border: '1px solid #F3F4F6', cursor: listing.slug ? 'pointer' : 'default' }}>
      {listing.image && (
      ...
      <div style={{ padding: '12px 14px 14px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8, marginBottom: 4 }}>
          <div style={{ fontSize: 16, fontWeight: 800, color: '#111827', lineHeight: 1.3 }}>
            {listing.name}
            {listing.verified && <img src="/verified-badge.svg" width={15} height={15} style={{ marginLeft: 5, verticalAlign: 'middle' }} alt="Verified" />}
          </div>
          {listing.price && (
        ...
        {listing.day && <div style={{ fontSize: 12, color: '#9CA3AF', marginTop: 3 }}>📅 {listing.day}</div>}
      </div>
    </div>
  )

  if (!listing.slug) return card
  return <Link href={`/listing/${listing.slug}`} style={{ textDecoration: 'none', color: 'inherit' }}>{card}</Link>
}
